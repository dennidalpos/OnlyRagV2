...
**Il piano non nasce da solo.** La UI fa tre cose in sequenza: genera (`agent:plan-generate`), semina in stato di sessione (`agent:plan-seed`), poi avvia l'agente sullo **stesso** `sessionId`. Saltare la semina significa eseguire senza piano — e senza piano metà dei guard non ha nulla su cui operare. `seedGeneratedPlan()` replica la sequenza.
