...
- L'autorità delle prove è stata corretta in W2.12. Restano recupero dopo conflitto versione, fallback dell'intervista e persistenza terminale: W2.13-W2.15 nel tracker.
...
Le prove live TS2305 e TS2613 del 9 settembre hanno entrambe seguito conflitto → lettura → edit riuscito, corretto l'import e concluso con `npm run build` verde e milestone 1/1 verificata. I processi Vitest sono però usciti con codice 1 sulla stessa asserzione `result.success === true`: la chiusura applicativa era `non_verificabile`, perché una build è prova strutturale e non end-to-end. Questo è il residuo W2.15; TS2614 non è stata avviata dopo due fallimenti consecutivi identici. W2.13 resta aperto fino al riallineamento della suite e alla conferma delle tre regressioni.
