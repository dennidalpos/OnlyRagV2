...
- Le diagnostiche descrivono la correzione come dato (`DiagnosticAdvice` in [`diagnosticAdvice.ts`](../electron/core/domain/agent/diagnosticAdvice.ts)): nel risultato del tool diventa un consiglio (`renderAdvice`, sezione "Suggested fix"), e solo l'arbitro la trasforma nell'ordine del turno (`renderOrder`, "Directives:" / "MUST"). L'arbitro rilegge i consigli salvati nella trascrizione con `parseRenderedAdvice`. Dettagli e stato della migrazione in [agent-guards.md](agent-guards.md#consigli-e-ordini).
