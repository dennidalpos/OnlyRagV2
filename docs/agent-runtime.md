...
- **Campionamento**: l'agente non invia `temperature`, `top_p`, `top_k`, `repeat_penalty`, `num_thread` né stop sequence; valgono i default del Modelfile e i core fisici scelti da Ollama. `modelSamplingOverrides` nelle impostazioni permette un override per modello ([`ollamaSamplingOptions.ts`](../shared/domain/agent/ollamaSamplingOptions.ts)).
