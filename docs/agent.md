...
Quando il controllo fallito è un test che ha girato e ha fallito un'asserzione (Jest, Vitest), [`testFailureDiagnostic.ts`](../electron/core/domain/agent/testFailureDiagnostic.ts) nomina il file di test e le righe Expected/Received del runner e ordina una sola riscrittura di quel file; la direttiva dell'arbitro riceve l'output del comando di verifica effettivo, non più solo quello di `npm run build`.
