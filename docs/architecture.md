...
- **Coding Studio Workhorse & Resilient Fallback Router**: Architettura basata su **Modello di Lavoro Fisso (Workhorse Model)** impostato dall'utente (`codingModel`, es. `qwen2.5-coder:7b`). Il modello rimane stabilmente allocato in VRAM per tutta la sessione garantendo il riutilizzo della KV-cache (zero VRAM thrashing). La valutazione della complessità (`evaluateTaskComplexity`) interviene unicamente per calibrare le direttive del prompt di sistema **family-agnostic** (`PromptCompiler.compileCodingPrompt`) senza mai alterare i pesi caricati in VRAM. Lo switch dei modelli è gestito esclusivamente in modo reattivo da `ResilientModelDispatcher` in caso di errore fisico fatale (`CUDA out of memory` o crash di Ollama), eseguendo l'evizione VRAM preventiva e il fallback sul modello leggero configurato (`codingFallbackModel`, es. `llama3.2:3b`).
