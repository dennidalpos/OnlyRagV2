...
- **Chiusura Applicativa Basata su Evidenze (`agentOrchestratorApplicationClosure.ts`, `agentOrchestratorVerificationRunner.ts`)**: `finish`, risposta finale in prosa, esaurimento budget ed errore di trasporto convergono nello stesso gate. L'applicazione esegue il controllo disponibile, conserva consegna parziale e motivo, persiste ed emette uno degli esiti `verified`/`unverifiable`/`blocked`/`cancelled`; dopo annullamento non avvia controlli. Un test comportamentale superato può produrre `verified`; build, typecheck, lint e presenza dei file restano evidenza strutturale e producono `unverifiable` rispetto al comportamento end-to-end. `finish` resta un segnale opzionale e può ottenere fino a tre cicli di correzione su un controllo fallito, ma non governa più la chiusura.
...
- **Anteprima non Equivale a Verifica (`browserPreviewVerification.ts`)**: `open_in_browser` raccoglie evidenza visiva ma non promuove milestone e non certifica il comportamento; una build o un typecheck dimostrano struttura, mentre l'esito terminale `verified` richiede un controllo comportamentale.
...
- **Generazione Piano Instradata (`planGenerationAppService.ts`)**: Il drafting del piano usa il canale IPC `agent:plan-generate`, applica la preferenza `modelContextLengths` al `num_ctx` del modello coding entro il tetto hardware e viene parsato con il parser canonico.
