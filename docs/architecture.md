...
- **Distinzione fra Ripetizione Riuscita e Ripetizione Fallita (`recordOutcome` / `classifyRepeatOutcome` + `resolveRedundantSuccessAction`)**:
  ...
  - Gli esiti sono indicizzati **sia per fingerprint sia per target**. Non tutte le regole del detector sono regole di fingerprint: il controllo di thrashing (`>= 4` modifiche allo stesso file) scatta sul PERCORSO, quindi un modello che riscrive lo stesso file con contenuto anche solo leggermente diverso produce ogni volta una fingerprint nuova. `classifyRepeatOutcome` rispondeva percio' `unknown`, l'esenzione non si applicava e la scala di stagnazione partiva comunque: nella sessione 4enx e' cosi' che m-3 e m-4 sono state abbandonate su scritture di `src/styles/globals.css` tutte riuscite, due step dopo un messaggio che diceva testualmente "this is NOT counted against you". La fingerprint esatta risponde per prima; se non ha memoria, risponde lo storico del target. Il loop di lettura ripetuta resta volutamente fuori dall'esenzione: una lettura riuscita non produce nulla, quindi escalare non butta via alcun deliverable.
...
- **Una Sessione che si Arrende non e' una Sessione Conclusa (`agentOrchestratorResponseInterpreter.ts`)**: tutto il gate sopra vive dentro il ramo `finish`, quindi vale zero se `finish` non viene mai invocato. In AGENT mode il modello che risponde in prosa riceve fino a 2 solleciti a emettere una tool call (`noToolStreak`); alla terza risposta non interpretabile il ramo di recupero non si applica piu' e il codice cadeva su `emitDone(true, ...)`. La sessione 4enx si e' chiusa cosi': `Status: COMPLETED` allo step 86, con quattro milestone abbandonate, quattro mai iniziate, nessun resoconto finale e la verifica finale mai eseguita — semplicemente perche' tre risposte di fila non erano parsabili. Ora quel ramo chiude `success: false` con un riassunto che nomina il piano rimasto aperto (`N/M milestone verificate, K abbandonate`) e conserva l'ultima risposta del modello nel log di audit. In ASK/PLAN mode il comportamento resta invariato: li' una risposta in prosa *e'* il deliverable.
  - Causa prossima di quella chiusura: il modello aveva emesso **tre** tool call native in un'unica risposta. `toolParser` prendeva la porzione di testo dalla prima graffa all'**ultima** graffa del messaggio, che e' un oggetto solo quando gli oggetti sono uno. `sliceBalancedObject` isola ora il primo oggetto bilanciato (tracciando stringhe ed escape, cosi' le graffe dentro il contenuto di un `write_file` non lo chiudono in anticipo) e la vecchia porzione greedy resta come fallback per un oggetto troncato: piu' chiamate in una risposta eseguono la prima invece di valere come nessuna chiamata.
- **Autorita' sugli Stati del Piano (`milestoneUpdateAuthority.ts`)**: `update_plan` era assoluto e qualsiasi stato dichiarato dal modello veniva scritto senza filtro. Un modello che si autodiagnostica male distruggeva il proprio piano (sessione o5fk: m-4 marcata `failed` con una nota copiata da un vecchio messaggio di loop intervention, m-5 riportata da `verified` a `in_progress` conservando la nota "Auto-verified" ormai falsa; quattro delle cinque milestone failed avevano i deliverable su disco). Vince l'evidenza su disco: un aggiornamento no-op viene respinto (costava un round-trip completo senza muovere nulla), una milestone `verified` non viene mai retrocessa (lo stato del piano non blocca alcun tool, quindi riaprirla non serve a rilavorare un file) e `failed` viene rifiutato finche' i deliverable citati nel titolo esistono con contenuto. Le note vengono sostituite, mai unite, perche' un cambio di stato invalida quanto il precedente diceva di se'.
  - `failed` ha due origini che non vanno confuse. Una milestone che ha fallito il **proprio comando di verifica** deve restare recuperabile: il modello corregge il codice, rilancia il controllo, passa. Una milestone **abbandonata dal loop guard** no: il guard le ha detto "stop working on it entirely" e ha spostato il focus proprio per rompere un loop, quindi lasciarci riscrivere sopra `verified` annulla l'escape. Nella sessione 4enx m-6 e' stata abbandonata allo step 41 e riportata verificata allo step 47. `abandonedMilestoneNote` / `isSystemAbandoned` distinguono i due casi dalla nota che il guard scrive, e un `update_plan` sulla milestone abbandonata riceve `[UPDATE_PLAN REJECTED: ABANDONED]`.
...
- **Prova Dichiarata in Fase di Pianificazione (`buildVerificationCommandsBlock` in `planGenerationAppService.ts` + `GoalDecompositionPlanner.extractVerificationDirective`)**: il prompt di pianificazione chiedeva milestone falsificabili ma non diceva *con cosa* verificarle, quindi il modello se le inventava — nella sessione o3tx tre milestone di verifica ("Run `npm run build`", "Run `tsc --noEmit`") su un progetto che non dichiarava nessuno dei due script. Il prompt riceve ora la lista **completa e chiusa** dei comandi che il workspace offre davvero, risolta da `projectVerificationResolver` sullo stesso manifest che il gate Definition of Done esegue al `finish`: cio' che la milestone dichiara come prova e cio' che il gate esegue sono la stessa stringa. Un workspace vuoto ne offre legittimamente zero, e allora il prompt lo dice esplicitamente (`VERIFICATION COMMANDS AVAILABLE IN THIS PROJECT: NONE.`) vietando di inventarne: le milestone che producono file sono provate dai path che scrivono, e solo uno script che il piano stesso dichiara in `package.json` puo' essere citato piu' avanti. Il `workspacePath` viaggia dal renderer (`usePlanApproval`) fino al servizio attraverso `agent:plan-generate`.
  - **Il comando dichiarato deve poter fallire e non deve scrivere (`verificationCommandSafety.ts`)**: il prompt vieta di inventare comandi, ma un 7B lo ignora, e il divieto vale quanto il codice che lo applica. Nella sessione 4enx il planner ha dichiarato come prova `touch src/App.tsx` e `echo "import React..." > src/pages/Tasks.tsx` per dieci milestone su quindici. Sono stati eseguiti: `touch` viene tradotto in `New-Item -Force`, che tronca un file esistente, e la redirezione ha riscritto i sorgenti dell'agente in UTF-16LE con i `\n` rimasti letterali. `src/App.tsx` e `src/pages/Tasks.tsx` hanno chiuso la sessione come TypeScript non valido, e il log ha registrato "Verification command passed" per il comando che li aveva appena distrutti. Due famiglie di rifiuto, entrambe fatali alla promessa che una verifica fa: **MUTANTE** (redirezioni `>`/`>>` fuori dalle stringhe, `touch`/`New-Item`/`Set-Content`/`cp`/`mv`/`rm`, `sed -i`, sottocomandi di scaffolding `init` e `create`) e **VACUO** (`echo`, `true`, `cd`, `Write-Host`: escono 0 qualunque sia lo stato del codice, quindi promuovere sul loro exit code e' il timbro automatico che il meccanismo esiste per eliminare). Build e test restano ammessi anche se scrivono in `dist/`: il loro exit code riflette il codice, che e' la proprieta' che conta. Il controllo e' applicato in tre punti — all'ingestione del piano (`parsePlanFromText` scarta il comando e lascia sulla milestone la nota che spiega perche'), prima dell'esecuzione in `agentOrchestratorPlanTool` (per i piani ripristinati da sessioni precedenti o modificati a mano: la milestone resta com'e' e il modello riceve `[UPDATE_PLAN REJECTED: VERIFICATION REFUSED]`), e sulla classificazione di un `run_command` come verifica in `trackVerification`, dove la ricerca per parole chiave da sola faceva passare `touch src/test.tsx` per un test e `echo "build ok" > out.log` per una build.
...
- **Smoke Test Automatico del Bundle Electron (`scripts/test_bundle_smoke.ps1`, `ONLYRAG_SMOKE_TEST=1`)**: I test unitari sotto Vitest importano i singoli file TypeScript e non intercettano bug di bundling o dipendenze con require dinamici risolti a runtime. Lo script di smoke test compila `dist-electron/main.js` ed avvia il runtime Electron in modalità headless controllata (`--smoke-test` / `ONLYRAG_SMOKE_TEST=1`). `main.ts` registra tutti i canali IPC ed i servizi di dominio, verifica l'assenza di crash al bootstrap ed emette `[SMOKE_TEST_PASS]` prima di uscire con codice 0. Integrato in `scripts/lint_format.ps1` (step 5/5) e `scripts/build_package.ps1` (step 3/5) per bloccare a monte qualsiasi regressione di bundling prima dell'impacchettamento finale.
- **Store Unificato delle Impostazioni su Filesystem (`appSettingsRepository.ts`, `settings.json`)**: Chromium isola e partiziona `localStorage` per URL Origin (`http://localhost:5173` in sviluppo vs `file://` nel pacchetto installato), causando divergenza silenziosa delle configurazioni (modelli attivi, wizard iniziale, profili hardware). Le preferenze applicative (`AppSettings`) sono gestite dal Main Process tramite `%APPDATA%/onlyrag-v2/settings.json` con scrittura atomica `.tmp+rename` e canali IPC `settings:get` e `settings:save`. All'avvio il renderer sincronizza le impostazioni dal file system con migrazione automatica one-shot del pregresso `localStorage`, garantendo parità al 100% tra dev e produzione.
