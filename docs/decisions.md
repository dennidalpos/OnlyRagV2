...
| Nessuna migrazione da layout precedenti: `settings.json` è accettato solo nel formato versionato e gli store di sessioni, progetti e stato agente solo nella struttura corrente. | Installazione mono-utente; la cronologia è stata azzerata il 2026-09-26 dopo aver portato ogni store al formato corrente. | `appSettingsRepository.ts` (`decodeSettingsFile`), `sessionHistoryRepository.ts`, `agentSessionStateRepository.ts`. |
