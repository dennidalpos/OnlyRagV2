...
* **`agentToolExecutorService.ts`**: Esegue in modo sicuro i tool invocati dal modello (`read_file`, `extract_code_symbols`, `write_file`, `replace_file_content`, `multi_replace_file_content`, `run_command`, `run_tests`, `list_dir`, `grep_search`, `copy_file`, `move_file`, `create_directory`, `git_status`, `git_diff`, `rollback_workspace`, `get_file_info`), registrando gli snapshot preventivi su `AtomicWorkspaceJournal`.
...
* **`agent/ollamaToolSchemaCatalog.ts`**: Catalogo JSON-Schema dei 24 tool dell'agente nel formato `tools` compatibile OpenAI/Ollama, derivato dai parametri reali accettati da `agentToolExecutorService.ts`, usato per instradare su `POST /api/chat` con tool-calling nativo quando il modello lo supporta.
