...
* **`usePlanApproval.ts`**:
  * **Responsabilità**: Gestisce il flusso di Plan Approval (drafting, versioning, approva/rifiuta, auto-proceed a countdown). Genera il piano via IPC `agent:plan-generate` (instradato attraverso il profilo hardware invece del `fetch()` diretto a Ollama usato in precedenza), allega i milestone canonici parsati dal backend (`GoalDecompositionPlanner`) al piano, e alla approvazione semina (`agent:plan-seed`) i milestone nello stato di sessione persistito prima dell'avvio dell'esecuzione. Include il residuo dei milestone non verificati del piano precedente come contesto di riconciliazione quando si genera un nuovo piano.
...
* **`agentOrchestratorAppService.ts`**: Esegue il loop agentico multi-step (Tool Calling), coordina lo streaming token per token verso la UI, calcola il `dynamicNumCtx`, intercetta loop ripetitivi con `AgentActionLoopDetector` (unica istanza, incorpora `CycleOscillationDetectorAndReproOracle` internamente), esegue il **Pre-Finish Verification Gate** per validare modifiche al codice prima della terminazione e gestisce l'auto-healing con rollback transazionale del filesystem. Integra l'escalation dinamica dei modelli su scatto del `StagnationCircuitBreaker` seguendo l'ordine dei tier (🟢 Fast → 🔵 Standard → 🟣 Deep Reasoning → 🔶 Heavy) con evizione atomica VRAM prima del cambio modello. Rileva la capacità di tool-calling nativo per modello (`supportsNativeToolCalling`) e instrada su `/api/chat` con il catalogo tool strutturato quando disponibile. Persiste lo stato di sessione (`persistCurrentState`) ogni N step e immediatamente dopo mutazioni file, garantendo comunque persistenza incondizionata su ogni percorso di uscita (finish/cancel/errore/timeout/circuit-breaker).
...
* **`agent/planManager.ts`**: Calcola lo stato compatto del piano (`CompactPlanState`) direttamente da `PlanMilestone[]` di `GoalDecompositionPlanner` (`getCompactStateFromMilestones`), senza round-trip di serializzazione/re-parsing markdown, e genera `.assistant/SESSION_TRACKER.md` (`generateSessionTrackerMarkdown`).
...
* **`agent/ollamaToolSchemaCatalog.ts`**: Catalogo JSON-Schema dei 25 tool dell'agente nel formato `tools` compatibile OpenAI/Ollama, derivato dai parametri reali accettati da `agentToolExecutorService.ts`, usato per instradare su `POST /api/chat` con tool-calling nativo quando il modello lo supporta.
