...
* **`agent/ollamaToolSchemaCatalog.ts`**: Catalogo JSON-Schema dei 23 tool dell'agente nel formato `tools` compatibile OpenAI/Ollama, derivato dai parametri reali accettati da `agentToolExecutorService.ts`, usato per instradare su `POST /api/chat` con tool-calling nativo quando il modello lo supporta.
