...
> [!NOTE]
> **Impatto sistema in tempo reale:** la formula $Footprint_{\text{Totale}} = W_{\text{mem}} + KV_{\text{mem}} + Overhead_{\text{CUDA}}$ (§3) e' calcolata da `assessModelHardwareCompatibility` in `hardwareRecommendationEngine.ts` e popola il campo `compatibilityStatus` di `ModelRecommendation` (`optimal_vram` / `tight_vram` / `exceeds_vram`). Il valore governa il ranking e la selezione dei modelli consigliati per ogni tier, ma **non e' esposto come badge nella UI**: il Setup Wizard (`WizardStepRecommendedModels.tsx`) elenca i modelli tramite `select` mostrando solo lo stato di download.
