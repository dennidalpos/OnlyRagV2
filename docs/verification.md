...
- `npm run test:e2e:settings-bootstrap` verifica con `userData` e `appData` isolati che la diagnostica attenda il caricamento delle impostazioni, che la migrazione one-shot preservi modello, lingua e wizard, e che le scritture ravvicinate vengano accorpate e serializzate. La precedenza di `settings.json` richiede ancora una prova senza handler sostitutivo.
...
- Il test di bootstrap impostazioni usa un handler IPC sostitutivo dopo la prima finestra: la precedenza del file reale prima della sostituzione e il rifiuto di `settings:get` richiedono ancora copertura. Se quella chiamata fallisce, la diagnostica automatica resta ferma.
