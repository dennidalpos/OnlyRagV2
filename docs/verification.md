...
- `npm run test:e2e:cold-start` avvia il bundle Electron con profilo isolato, intercetta e blocca le richieste esterne Chromium e rimonta il Renderer. Verifica che il rimontaggio dell'interfaccia non richieda Google Fonts, il registro Ollama, GitHub o altri host esterni. La diagnostica con impostazioni locali interroga gli endpoint locali predefiniti; controlli aggiornamenti modello e Skill Hub partono da azioni utente. L'intercettazione non copre ancora il primo avvio Main.
