...
- La prova completa `fullTaskRun.live.ts` usa come riferimento `qwen3.8:27b` sull'hardware Windows disponibile; `ONLYRAG_LIVE_MODEL` permette una diagnosi esplicita con un altro modello. Il timeout live è di 180 minuti. Nel workspace isolato il probe approva una richiesta alla volta solo per `npm install` e `npm install --save-dev vitest`, concordati per questa qualifica; ogni altra richiesta di rete viene respinta. La qualifica richiede la chiusura verificata del task e milestone provate, senza soglia di velocità: l'offload su RAM e CPU è previsto.
