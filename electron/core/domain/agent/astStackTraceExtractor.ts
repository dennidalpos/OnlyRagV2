 ...
 */
export class ASTAwareStackTraceExtractor {
  private static ERROR_HEADER_REGEX = /(?:Error|Exception|TypeError|ReferenceError|SyntaxError|FAIL|Failed): (.*)/i
  private static FILE_LINE_REGEX = /(?:at\s+(?:.*?\()|at\s+|in\s+|^|\s)([a-zA-Z]:[\\/][^\s:]+\.(?:ts|tsx|js|jsx|py|json)|[a-zA-Z0-9_./\\-]+\.(?:ts|tsx|js|jsx|py|json)):(\d+)(?::(\d+))?/i
  ...
  public static extractErrorDiagnostics(rawTerminalOutput: string): ExtractedErrorFrame | null {
    ...
    for (const line of lines) {
      const headerMatch = line.match(this.ERROR_HEADER_REGEX)
      ...
      const fileMatch = line.match(this.FILE_LINE_REGEX)
