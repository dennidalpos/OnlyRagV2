...
export const capabilityOperationSchema = z.enum(['read', 'write', 'delete', 'execute', 'connect', 'download', 'commit', 'open'])

export const capabilityConsentSchema = z
  .object({
    requested: z.boolean(),
    granted: z.boolean(),
...
export const capabilityPolicyRequestSchema = z
  .object({
...
export const capabilityPolicyDecisionSchema = z
  .object({
    allowed: z.boolean(),
...
export const capabilityPolicyAuditEventSchema = z
  .object({
    auditId: nonBlank.max(200),
    sessionId: nonBlank.max(200),
