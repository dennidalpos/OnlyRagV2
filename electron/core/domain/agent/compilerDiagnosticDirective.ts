...
import { buildBrowserGlobalAdvice, buildTestFailureAdvice, domEnvironmentFor, extractFailingTest, testedModuleText } from './testFailureDiagnostic'
...
export function buildDeferredDiagnosticNote(output: string): string | null {
  ...
  return [
    `\n\n[ALSO REPORTED, AFTER THE FIX ABOVE]`,
    `The same output carries ${codeErrors.length} error${codeErrors.length === 1 ? '' : 's'} the fix above does not address. ${codeErrors.length === 1 ? 'It is' : 'They are'} best left for a later step: ${codeErrors.length === 1 ? 'it' : 'they'} will still be reported after that fix, and then the file to edit is named here:`,
...
function buildMissingScriptProgramAdvice(missing: MissingScriptProgram, facts: DiagnosticWorkspaceFacts): DiagnosticAdvice {
  const heading = `[THE SCRIPT'S PROGRAM IS NOT INSTALLED — "${missing.program}"]`
  const observed = `"npm run ${missing.script}" runs "${missing.body}", but no installed package provides the "${missing.program}" command. Rewriting source files cannot fix this.`
  const rerun = `Then run "npm run ${missing.script}" again.`
  if (VITE_REPLACEABLE.has(missing.program) && facts.binaryInstalled?.('vite')) {
    return diagnosticAdvice(
      heading,
      [observed],
      `"write_file" on "package.json": the same complete file with the "${missing.script}" script changed to exactly "${viteScriptFor(missing.script)}". Vite is installed and this project is laid out for it; keep every other line.`,
      [rerun],
    )
  }
  return diagnosticAdvice(
    heading,
    [observed],
    `"run_command" with the command: npm install --save-dev ${PROGRAM_PACKAGES[missing.program] ?? missing.program}`,
    [rerun],
  )
}
...
function buildBundlerMissingExportAdvice(missing: BundlerMissingExport, exportedNames: string[]): DiagnosticAdvice {
  ...
  return diagnosticAdvice(
    ...
    `"write_file" on "${exporter}": ${fix}. Keep every other line.`,
    [`Do NOT re-read "${exporter}" and do NOT re-run the build until that file has changed.`],
  )
}
...
function buildUnresolvedBundlerImportAdvice(unresolved: UnresolvedBundlerImport, facts: DiagnosticWorkspaceFacts): DiagnosticAdvice {
  const fix = unresolvedBundlerImportFix(unresolved, facts)
  const heading = `[THE IMPORTED FILE DOES NOT EXIST — "${unresolved.importer}" imports "${unresolved.specifier}"]`
  const observed = [`The bundler resolved "${unresolved.specifier}" from the folder of "${unresolved.importer}" and found nothing there.`]
  if (fix.kind === 'redirect') {
    return diagnosticAdvice(
      heading,
      observed,
      `"write_file" on "${unresolved.importer}": the same complete file with "${unresolved.specifier}" changed to "${fix.specifier}", which exists.`,
  ...
  if (fix.kind === 'remove') {
    return diagnosticAdvice(
      heading,
      observed,
      `"write_file" on "${unresolved.importer}": the same complete file without the line that imports "${unresolved.specifier}". A stylesheet import binds nothing, so no other line changes; the project's styles are already imported by its entry.`,
      ['Do NOT edit any other stylesheet for this error. Then run the build again.'],
    )
  }
  return diagnosticAdvice(
    heading,
    observed,
    `"write_file" on "${fix.path}", creating that file with the exports "${unresolved.importer}" imports from "${unresolved.specifier}".`,
    ['Then run the build again.'],
  )
}
...
function buildCssImportedScriptAdvice(file: string, packageName: string): DiagnosticAdvice {
  const fix =
  ...
  return diagnosticAdvice(
    ...
    `"write_file" on "${file}": the same complete file with ${fix}. Keep every other line.`,
    ['Then run the build again.'],
  )
}

function buildCssSyntaxAdvice(failure: CssSyntaxFailure, facts: DiagnosticWorkspaceFacts): DiagnosticAdvice {
  ...
  if (importedPackage) return buildCssImportedScriptAdvice(file, importedPackage)
  return diagnosticAdvice(
    ...
    `"write_file" on "${file}": the complete file as valid CSS${failure.line ? `, with line ${failure.line} corrected or removed` : ''}.`,
    ['Then run the build again.'],
  )
}

function buildUnresolvedCssImportAdvice(cssImport: UnresolvedCssImport, facts: DiagnosticWorkspaceFacts): DiagnosticAdvice {
...
export function buildDiagnosticFixAdvice(
  output: string,
  ...
  const cssSyntax = extractCssSyntaxFailure(output)
  if (cssSyntax) return buildCssSyntaxAdvice(cssSyntax, facts)
  const bundlerImport = extractUnresolvedBundlerImport(output)
  if (bundlerImport) return buildUnresolvedBundlerImportAdvice(bundlerImport, facts)
  const bundlerExport = extractBundlerMissingExport(output)
  if (bundlerExport) {
    const exporterFile = bundlerExport.exporter.split('/').pop() ?? bundlerExport.exporter
    return buildBundlerMissingExportAdvice(bundlerExport, resolveLocalModuleExports(bundlerExport.exporter, `./${exporterFile}`))
  }
  ...
  if (jsxInScript) {
    return diagnosticAdvice(
      ...
      `"move_file" with sourcePath "${jsxInScript.file}" and targetPath "${jsxInScript.renamedFile}". Keep the content unchanged.`,
      [
        `Imports without an extension (e.g. "./App") resolve to the renamed file. Only if a file imports "${jsxInScript.file.split('/').pop()}" with its extension, rewrite that import line.`,
        `Then run the build again.`,
      ],
    )
  }
  ...
  if (all.length === 0) {
    ...
    if (failingTest.browserGlobal) return buildBrowserGlobalAdvice(failingTest, domEnvironmentInstalled(failingTest, facts))
    ...
    const moduleText =
      failingTest.kind === 'assertion' && readWorkspaceFile && readLocalModuleSource
        ...
        : null
    return buildTestFailureAdvice(failingTest, (importingFile, specifier) => resolveLocalModuleExports(importingFile, specifier).length > 0, moduleText)
  }
  ...
  if (diagnostics.length === 0) {
    ...
    return diagnosticAdvice(
      ...
      `"run_command" with: npm install --save-dev typescript@latest`,
      [`Do NOT edit any file under node_modules, and do NOT rewrite your own source for these errors.`],
    )
  }
  ...
  if (suggested) {
    return diagnosticAdvice(
      ...
      `"run_command" with the command: ${suggested}`,
      [`Do NOT rewrite "${first.file}" for this error. It will report the same thing until the package is installed.`],
    )
  }
  ...
  if (mismatch) {
    ...
    if (localModule) {
      ...
      if (localExports.includes('default') && /^import\s+[^'{]+\s+from\s+/.test(mismatch.suggestedImport)) {
        return diagnosticAdvice(
          ...
          `"write_file" on "${target.file}", with the complete content of that file, replacing line ${target.line} with exactly: ${mismatch.suggestedImport}`,
          [`Do NOT edit "${importedTarget}" and do NOT re-run the command until the importer has changed.`],
        )
      }
      return diagnosticAdvice(
        ...
        `"write_file" on exactly one side, preserving the task's intended public API; do not change both sides merely to silence the compiler.`,
        [`Do NOT re-run the command until you have changed a file. It will report exactly these errors again, because nothing will have changed.`],
      )
    }
    ...
    return diagnosticAdvice(
      ...
      `"write_file" on "${target.file}", with the complete content of that file, in which line ${target.line} is replaced by exactly: ${mismatch.suggestedImport}`,
      [`Do NOT re-run the command until you have changed a file. It will report exactly these errors again, because nothing will have changed.`],
    )
  }
  ...
  if (missingRelative) {
    ...
    return diagnosticAdvice(
      ...
      `"write_file" on "${missingRelative.expectedPath}", creating that file with the exports "${target.file}" imports from "${missingRelative.specifier}".`,
      [`Do NOT rewrite "${target.file}", and do NOT re-run the command until that file exists.`],
    )
  }
  ...
  if (missingLocalExport) {
    ...
    return diagnosticAdvice(
      ...
      available.length > 0
        ? `"write_file" on "${target.file}", importing only names from the list above, or add "${missingLocalExport.memberName}" to the local module if that is the API the task requires.`
        : `"write_file" on "${target.file}", removing or correcting the unsupported "${missingLocalExport.memberName}" import.`,
      [`Do NOT re-run the command with the same import; the local module does not currently export that name.`],
    )
  }
  ...
  if (missingExport) {
    ...
    return diagnosticAdvice(
      ...
      available.length > 0
        ? `"write_file" on "${target.file}", importing only names from the list above, or building "${missingExport.memberName}" yourself with plain JSX instead of importing it.`
        : `"write_file" on "${target.file}", building "${missingExport.memberName}" yourself with plain JSX instead of importing it from "${missingExport.packageName}".`,
      [`Do NOT re-import "${missingExport.memberName}" from "${missingExport.packageName}", and do NOT install another package to obtain it.`],
    )
  }
  ...
  return diagnosticAdvice(
    ...
    `"write_file" on "${first.file}", with the complete corrected content of that file.`,
    [`Do NOT re-run the command until you have changed a file. It will report exactly these errors again, because nothing will have changed.`],
  )
}
