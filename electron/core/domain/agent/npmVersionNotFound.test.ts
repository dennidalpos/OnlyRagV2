...
import { parseVersionNotFound, buildVersionNotFoundNote } from './npmVersionNotFound'
...
describe('buildVersionNotFoundNote', () => {
  it('names the version the registry actually publishes', () => {
    const directive = buildVersionNotFoundNote({ packageName: '@types/react', requestedRange: '^19.3.5' }, '19.2.18')
  ...
  it('never invents a number when the registry could not be reached', () => {
    ...
    const directive = buildVersionNotFoundNote({ packageName: '@types/react', requestedRange: '^19.3.5' })
