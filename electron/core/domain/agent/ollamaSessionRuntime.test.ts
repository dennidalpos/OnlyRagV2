...
describe('restored Ollama runtime validation', () => {
  it('accepts the exact endpoint, model tag, and digest', () => {
    expect(
      validateRestoredOllamaRuntime(profile, '127.0.0.1:11434/', [profile.model], { [profile.model]: { capabilities: [], digest: profile.digest } }),
    ).toBeNull()
  })
  ...
  it('rejects endpoint, model, or digest drift', () => {
    ...
    expect(
      validateRestoredOllamaRuntime(profile, profile.host, [profile.model], {
  ...
  it('rejects malformed persisted options', () => {
    expect(
      validateRestoredOllamaRuntime({ ...profile, options: { ...profile.options, num_ctx: 0 } }, profile.host, [profile.model], {
