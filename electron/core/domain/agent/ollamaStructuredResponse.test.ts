...
import { interviewPhaseResponseSchema, planningPhaseResponseSchema, toOllamaJsonSchema, validateStructuredContent } from './ollamaStructuredResponse'

describe('Ollama structured phase responses', () => {
  it('accepts the explicit no-question interview result', () => {
    const result = validateStructuredContent('{"hasQuestions":false,"questions":[]}', interviewPhaseResponseSchema)
  ...
  it('rejects duplicate question IDs and options', () => {
    ...
    const duplicateIds = validateStructuredContent(
      JSON.stringify({ hasQuestions: true, questions: [validOptions, validOptions] }),
      interviewPhaseResponseSchema,
    )
  ...
  it('rejects plan interventions without evidence', () => {
    const result = validateStructuredContent(
      JSON.stringify({
        ...
        assumptions: [],
        interventions: [{ id: 'm-1', objective: 'Inspect code', filePaths: [], acceptanceCriteria: ['Known result'] }],
        supersededWork: [],
      }),
