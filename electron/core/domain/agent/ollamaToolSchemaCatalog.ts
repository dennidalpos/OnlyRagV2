...
export const OLLAMA_TOOL_SCHEMA_CATALOG: OllamaToolSchema[] = [
  ...
  tool(
    ...
    'Extract functions, classes, and interfaces declared in a source file.',
    {
  ...
  tool(
    'list_dir',
    'List the immediate contents (files and directories) of a directory.',
    {
  ...
  tool(
    ...
    'Recursively list files and directories up to a depth limit.',
    {
  ...
  tool('run_tests', 'Run the workspace test suite and return a structured pass/fail summary instead of raw terminal output.', {
    command: {
      type: 'string',
      description:
        'Optional explicit test command override (e.g. "pytest -k test_login"). If omitted, auto-detected from the workspace (package.json test script or pytest config).',
    },
  }),
  tool(
    'inspect_os_env',
  ...
  tool(
    'git_commit',
    'Stage all workspace changes and create a git commit. Always requires explicit user approval before it runs, regardless of agent mode.',
    {
  ...
  tool(
    'get_file_info',
    'Get metadata about a file: existence, size, line count, binary detection.',
    {
  ...
  tool(
    'ensure_tool',
    'Ensure a development tool is installed on the host, installing it if missing. Only node, npm, pnpm, git, python and ollama can be installed.',
    {
  ...
  tool(
    'ask',
    'Ask the user a clarifying question and pause execution for their answer.',
    {
  ...
  tool(
    ...
    'Render a local HTML/web artifact in a headless browser and capture screenshot, DOM, console errors, and failed HTTP responses as validation evidence.',
    {
