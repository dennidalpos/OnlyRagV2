...
describe('test command detection', () => {
  it('prefers the fast npm script', () => {
    expect(detectTestCommand('workspace', () => ({ test: 'vitest', 'test:fast': 'vitest --run' }), noPytest)).toEqual({
      command: 'npm run test:fast',
      source: 'package.json scripts["test:fast"]',
    })
  })

  it('falls back to the standard npm test script', () => {
    expect(detectTestCommand('workspace', () => ({ test: 'vitest' }), noPytest)).toEqual({ command: 'npm test', source: 'package.json scripts.test' })
  })
