...
describe('executeListDirectoryTool', () => {
  it('lists entries through the repository boundary', () => {
    const result = executeListDirectoryTool({ dirPath: 'src' }, 'C:/workspace', {
      listDirEntries: () => [
  ...
  it('rejects paths outside the workspace', () => {
    const result = executeListDirectoryTool({ dirPath: '../secrets' }, 'C:/workspace', { listDirEntries: () => [] })

    expect(result.outputForHistory).toContain('Security Violation')
  })

  it('reports a missing directory without throwing', () => {
    const result = executeListDirectoryTool({ dirPath: 'missing' }, 'C:/workspace', { listDirEntries: () => null })
