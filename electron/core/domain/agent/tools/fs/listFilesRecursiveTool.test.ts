...
describe('executeListFilesRecursiveTool', () => {
  it('limits depth and delegates ignored-directory policy', () => {
    ...
    const result = executeListFilesRecursiveTool({ dirPath: 'src', maxDepth: 99 }, 'C:/workspace', {
      exists: () => true,
  ...
  it('rejects paths outside the workspace', () => {
    const result = executeListFilesRecursiveTool({ dirPath: '../secrets' }, 'C:/workspace', { exists: () => true, listRecursive: () => [] })
