...
/** live-full-task, 2026-08-24: milestone m-10 was "Create `src/services` folder" — a directory, so `extractDeliverablePaths` finds nothing and the milestone resolves `not_applicable`. */
describe('shouldDirectUnprovableClosure', () => {
  ...
  // A milestone whose files are genuinely missing must keep asking for them: telling the model to close it on its own judgement is exactly the fabricated verification the promotion rules exist to prevent.
  ...
  /** `update_plan` RUNS a declared verificationCommand and promotes on its exit code, so a command demonstrably CAN prove such a milestone. */
