...
try {
    ...
    try {
        ...
        Write-Host "[OK] Processi verificati." -ForegroundColor Green
    } catch {
        Write-Host "[WARN] Nessun processo da arrestare." -ForegroundColor Gray
    }
    ...
    if ($Mode -eq "Repo" -or $Mode -eq "Full") {
        ...
        foreach ($target in $targetsRepo) {
            if (Test-Path $target) {
                Write-Host "Rimuovendo: $target" -ForegroundColor Gray
                Remove-Item -Path $target -Recurse -Force -ErrorAction SilentlyContinue
            }
        }

        Get-ChildItem -Path $rootDir -Recurse -Filter "*.pyc" -ErrorAction SilentlyContinue | ForEach-Object {
            Remove-Item -Path $_.FullName -Force -ErrorAction SilentlyContinue
        }

        Write-Host "[OK] Pulizia repository completata." -ForegroundColor Green
    }
    ...
    if ($Mode -eq "UserData" -or $Mode -eq "Full") {
        ...
        foreach ($userDataPath in $targetsUserData) {
            if (Test-Path $userDataPath) {
                Write-Host "Rimuovendo dati utente: $userDataPath" -ForegroundColor Gray
                Remove-Item -Path $userDataPath -Recurse -Force -ErrorAction SilentlyContinue
            }
        }

        Write-Host "[OK] Pulizia dati utente locali completata." -ForegroundColor Green
    }
