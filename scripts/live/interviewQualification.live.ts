...
describe('live: interview qualification', () => {
  it('skips clear work and carries explicit custom answers into planning', async () => {
    ...
    const answers: UserInterviewAnswer[] = interview.questions.map((question, index) =>
      index === 0
        ? {
    ...
    const effectivePrompt = agentInterviewAppService.enrichPromptWithAnswers(ambiguous, answers, interview.questions)
