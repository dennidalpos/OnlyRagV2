...
export function resolveModelContextLength(
  ...
  const hardwareValue = Number.isFinite(hardwareDefault) ? Math.max(MIN_MODEL_CONTEXT_LENGTH, Math.floor(hardwareDefault)) : MIN_MODEL_CONTEXT_LENGTH
  const preferred = preferences?.[model]
  const knownModelMaximum = trainedContext && Number.isFinite(trainedContext) && trainedContext > 0 ? Math.floor(trainedContext) : undefined
  ...
  const modelMinimum = knownModelMaximum ? Math.min(MIN_MODEL_CONTEXT_LENGTH, knownModelMaximum) : MIN_MODEL_CONTEXT_LENGTH
...
export function getModelContextChoices(modelMaximum?: number): number[] {
  const knownMaximum = Number.isFinite(modelMaximum) && (modelMaximum as number) > 0 ? Math.floor(modelMaximum as number) : undefined
  const minimum = knownMaximum ? Math.min(MIN_MODEL_CONTEXT_LENGTH, knownMaximum) : MIN_MODEL_CONTEXT_LENGTH
  const maximum = knownMaximum ? knownMaximum : MIN_MODEL_CONTEXT_LENGTH
