...
- Scripts reside in `scripts/`, enforce `$ErrorActionPreference = "Stop"`, and output UTF-8 encoding:
  ...
  - `scripts/audit_codebase.ps1`: Performs serial architectural audits using `dpdm` (cycles), `knip` (dead code / orphan exports), and `skott` (graph analyzer & Web UI). Supports `-Fast`, `-Mode [All|Cycles|DeadCode|Graph]`, and `-WebUI`.
