...
- **Resilient SLM Log Diagnostics**: Dual-engine architecture with FastAPI Python sidecar scanner and native Node.js Electron fallback (`sidecarSlmBridgeService.ts`) for zero-downtime anomaly detection (CUDA OOM, truncated JSON, tool loop, timeouts) and actionable remediation steps.
