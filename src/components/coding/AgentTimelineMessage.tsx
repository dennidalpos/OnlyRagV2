...
import { ChevronDown, ChevronRight, User, Bot, AlertTriangle, CheckCircle2, XCircle, Terminal, Globe, FolderTree, Copy, Check } from 'lucide-react'
...
function formatInlineMarkdown(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith('`') && part.endsWith('`') && part.length >= 2) {
      return (
        <code key={i} className="px-1.5 py-0.5 rounded bg-slate-950 text-cyan-300 font-mono text-[11px] border border-slate-800">
          {part.slice(1, -1)}
        </code>
      )
    }
      ...
      return (
        <div
          className={`py-1.5 px-2.5 rounded-lg border text-xs space-y-1 shadow-sm font-sans ${
            isLoopWarning ? 'bg-amber-950/30 border-amber-500/40 text-amber-200' : 'bg-rose-950/30 border-rose-500/40 text-rose-200'
          }`}
        >
          <div className="flex items-center justify-between text-[10px]">
            <div className="flex items-center gap-1.5 font-bold">
              <AlertTriangle className={`w-3.5 h-3.5 ${isLoopWarning ? 'text-amber-400' : 'text-rose-400'}`} />
              <span className="uppercase tracking-wide">{isLoopWarning ? 'Loop Intervention' : 'System Alert'}</span>
            </div>
        ...
        <div className="space-y-1 font-mono text-[11px]">
          ...
          >
            <div className="flex items-center gap-2 min-w-0">
              {isPass ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" /> : <XCircle className="w-3.5 h-3.5 text-rose-400 shrink-0" />}
        ...
        <div className="space-y-1 font-mono text-[11px]">
          <div className="flex items-center justify-between py-1 px-2.5 rounded-lg bg-slate-900/60 border border-slate-800/80 group">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className={`px-1 py-0.2 rounded text-[9px] font-bold shrink-0 ${badge.color}`}>{badge.label}</span>
              <button
                type="button"
                onClick={() => {
      ...
      <div className="space-y-1 font-sans text-xs">
        ...
        >
          ...
          <div className="flex items-center gap-2 shrink-0">
            <span className="px-1.5 py-0.2 rounded bg-slate-900 text-slate-400 border border-slate-800 text-[9px] font-mono">{modelTag}</span>
            <span className="text-[10px] text-slate-400 font-mono">{formatClockTime(log.timestamp)}</span>
            {isInteractive && (
              <span className="text-slate-400">{isExpanded ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronRight className="w-3.5 h-3.5" />}</span>
            )}
          </div>
        </div>
