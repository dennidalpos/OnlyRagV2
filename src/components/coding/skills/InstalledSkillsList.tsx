  ...
  if (skills.length === 0) {
    return (
      <div className="text-center py-12 text-slate-500 space-y-4">
        ...
        <div>
          <p className="text-sm font-medium text-slate-300">Nessuna skill installata nel workspace.</p>
          <p className="text-xs text-slate-500">Crea una skill personalizzata o scaricane una dall'Hub.</p>
        </div>
        <div className="flex items-center justify-center gap-3">
          ...
          >
            <Plus className="w-3.5 h-3.5 text-cyan-400" /> Crea Nuova Skill
          </button>
          <button
          ...
          >
            <Globe className="w-3.5 h-3.5" /> Sfoglia Skill Hub
          </button>
        </div>
      </div>
    )
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            placeholder="Filtra skill installate per nome o trigger..."
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 bg-slate-950 p-1 rounded-xl border border-slate-800">
            {[
              { id: 'all', label: 'Tutte' },
              { id: 'hub_original', label: 'Originali' },
              { id: 'hub_modified', label: 'Modificate' },
              { id: 'local_custom', label: 'Locali' },
            ].map((f) => (
              <button
                key={f.id}
      ...
      <div className="space-y-3">
        {filteredSkills.length === 0 ? (
          <div className="text-center py-8 text-slate-500 text-xs">
            Nessuna skill corrisponde al filtro impostato.
          </div>
        ) : (
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {skill.originType === 'hub_original' && (
                    <span className="text-[10px] px-2 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-medium flex items-center gap-1">
                      <Globe className="w-3 h-3" /> Hub Originale ({skill.originHub || 'Sito'})
                    </span>
                  )}
                  {skill.originType === 'hub_modified' && (
                    <span className="text-[10px] px-2 py-0.5 rounded bg-amber-500/15 border border-amber-500/40 text-amber-300 font-medium flex items-center gap-1">
                      <AlertTriangle className="w-3 h-3" /> Modificata (non più originale del sito)
                    </span>
                  )}
                  {skill.originType === 'local_custom' && (
                    <span className="text-[10px] px-2 py-0.5 rounded bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 font-medium flex items-center gap-1">
                      <User className="w-3 h-3" /> Locale Personalizzata
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-1.5 self-end sm:self-start shrink-0">
                <button
                  ...
                  title={skill.isActive ? 'Skill sempre inclusa' : 'Inclusione automatica per matching'}
                >
                  <CheckCircle className="w-3 h-3" />
                  {skill.isActive ? 'Attiva' : 'Auto-Match'}
                </button>
                ...
                {skill.originType === 'hub_modified' && (
                  <button
                    ...
                    title="Ripristina contenuto originale dal sito/hub"
                  >
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              {skill.triggers.map((t) => (
                <span key={t} className="text-[10px] px-2 py-0.5 rounded bg-slate-900 border border-slate-800 text-slate-300">
                  trigger: {t}
                </span>
              ))}
            </div>
            ...
            <div className="pt-2 border-t border-slate-800/60 flex items-center justify-between">
              ...
              >
                {expandedSkillId === skill.id ? 'Nascondi Linee Guida' : 'Mostra Linee Guida Markdown'}
              </button>
