...
export const HardwareSetupWizardModal: React.FC<HardwareSetupWizardModalProps> = ({
  ...
  const [selectedTranslation, setSelectedTranslation] = useState<string>(settings.translationModel || recommendedSuite.translation)
  const [selectedMedical, setSelectedMedical] = useState<string>(settings.medicalModel || '')
  const [selectedLegal, setSelectedLegal] = useState<string>(settings.legalModel || '')
  const [selectedVision, setSelectedVision] = useState<string>(settings.visionModel || recommendedSuite.vision)
  ...
  useEffect(() => {
    if (isOpen && !prevIsOpenRef.current) {
      if (settings.codingModel || settings.defaultModel) {
        setSelectedCoding(settings.codingModel || settings.defaultModel)
      }
  ...
  const uniqueSelectedModels = Array.from(
    new Set([selectedCoding, selectedChat, selectedTranslation, selectedMedical, selectedLegal, selectedVision, selectedEmbedding]),
