...
export const VectorSearchModal: React.FC<VectorSearchModalProps> = ({ isOpen, onClose, embeddingModel = 'nomic-embed-text' }) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-5 border-b border-slate-800 flex items-center justify-between bg-slate-950/60 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">{t('vectorSearch.modalSubtitle')}</p>
          </div>
        </div>
      ...
      <div className="p-5 border-b border-slate-800 bg-slate-950/40 space-y-3 shrink-0">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
            ...
            <div className="flex items-center bg-slate-950 rounded-lg border border-slate-800 p-0.5" role="group" aria-label={t('vectorSearch.topKResults')}>
              {[3, 5, 10, 20].map((k) => (
                <button
                  ...
                  onClick={() => setTopK(k)}
                  aria-pressed={topK === k}
                  className={`px-2.5 py-1 rounded-md text-[11px] font-mono font-semibold transition-all focus-ring ${
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="px-2.5 py-1 rounded-lg bg-slate-950 border border-slate-800 text-cyan-300 font-mono text-[11px] font-bold">{embeddingModel}</span>
          </div>
        </div>
      </div>
      ...
      {/* Results Stream Area */}
      <div
        className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-950/60 select-text"
        tabIndex={0}
        aria-label={t('vectorSearch.resultsFound', { count: results.length })}
      >
        {isSearching && (
        ...
        {!isSearching && !error && !hasSearched && (
          <div className="h-full flex flex-col items-center justify-center text-center p-12 space-y-3 text-slate-400">
            ...
            <p className="text-xs max-w-md text-slate-400 leading-relaxed">{t('vectorSearch.modalSubtitle')}</p>
          </div>
        )}
        ...
        {!isSearching && !error && hasSearched && results.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center p-12 space-y-2 text-slate-400">
            ...
            <p className="text-xs max-w-md text-slate-400">{t('vectorSearch.noResultsHint')}</p>
          </div>
        )}
                ...
                <div key={idx} className="p-4 bg-slate-900/80 rounded-xl border border-slate-800 hover:border-cyan-500/40 transition-all space-y-2.5 shadow-md">
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2 truncate">
                      <span className="px-2 py-0.5 rounded bg-slate-950 border border-slate-800 text-cyan-300 font-mono text-[10px] font-bold">#{idx + 1}</span>
                      <span className="font-bold text-slate-200 truncate">{r.doc_name || t('vectorSearch.docName')}</span>
                      {r.chunk_id && <span className="text-[10px] text-slate-400 font-mono hidden md:inline">[{r.chunk_id}]</span>}
                    </div>
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-950/60 flex items-center justify-between text-xs shrink-0">
        <span className="text-slate-400 font-mono text-[11px]">LanceDB Embedded Vector Database • Hybrid Search Engine</span>
        <button
          type="button"
          onClick={onClose}
