...
export const SettingsView: React.FC<SettingsViewProps> = React.memo(
  ({ diagnostics, settings, onUpdateSettings, onRefreshDiagnostics, onOpenAboutModal, onOpenWizard, isActive = true }) => {
    ...
    const { updateAvailableMap, isCheckingUpdates, checkForUpdates, triggerUpdateModel, isModelUpdating, isAnyModelUpdating, downloadProgress } =
      ...
      <div className="flex-1 h-full overflow-y-auto p-8 space-y-9 bg-slate-950 select-text">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-5">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 mt-0.5">{t('settings.description')}</p>
            </div>
          </div>
        </div>
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2.5 px-1">
            ...
            <h2 className="text-sm font-bold uppercase tracking-wider text-slate-300">1. {t('settings.generalPreferencesSection')}</h2>
          </div>
              ...
              <div className="flex items-center gap-2 bg-slate-950 p-1 rounded-xl border border-slate-800/80">
                <button
                  ...
                  className={`flex-1 py-1.5 rounded-lg text-xs font-semibold transition-all focus-ring ${
                    language === 'it' ? 'bg-cyan-600 text-slate-950 shadow-md shadow-cyan-950/50' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900'
                  }`}
                >
                  🇮🇹 Italiano
                </button>
                <button
                  ...
                  className={`flex-1 py-1.5 rounded-lg text-xs font-semibold transition-all focus-ring ${
                    language === 'en' ? 'bg-cyan-600 text-slate-950 shadow-md shadow-cyan-950/50' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900'
                  }`}
                >
                  🇬🇧 English
                </button>
          ...
          <div className="glass-panel rounded-xl p-5 border border-slate-800 space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-800/80 pb-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-100">{t('settings.systemDirectoriesTitle')}</h3>
                <p className="text-xs text-slate-400">{t('settings.systemDirectoriesDesc')}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
              ...
              <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 space-y-2.5">
                ...
                <p className="text-[11px] text-slate-400 leading-tight">{t('settings.translationOutputFolderDesc')}</p>
              ...
              <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 space-y-2.5">
                ...
                <p className="text-[11px] text-slate-400 leading-tight">{t('settings.appLogsFolderDesc')}</p>
                <div className="text-[11px] font-mono text-slate-300 bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1.5 break-all">
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between px-1">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-sm font-bold uppercase tracking-wider text-slate-300">2. {t('settings.hardwareRuntimeSection')}</h2>
            </div>
            <button
              type="button"
          ...
          <OllamaServerConfig settings={settings} onUpdateSettings={onUpdateSettings} onRefreshDiagnostics={onRefreshDiagnostics} />

          {/* OCR Engine Selector */}
          <OcrEngineSelector settings={settings} onUpdateSettings={onUpdateSettings} />
        </section>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between px-1">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-sm font-bold uppercase tracking-wider text-slate-300">3. {t('settings.modelsWeightsSection')}</h2>
            </div>
            <button
              type="button"
          ...
          <ModelAssignmentGrid settings={settings} diagnostics={diagnostics} onUpdateSettings={onUpdateSettings} />
                      ...
                      >
                        ...
                        <div className="space-y-1.5">
                          ...
                          {isRunning && (
                            <div className="space-y-1 text-[10px] font-mono text-emerald-400/90 bg-emerald-950/30 px-2 py-1 rounded-lg border border-emerald-900/40">
                              {memory && (
                                <div>
                                  GPU: {(memory.gpuBytes / 1024 ** 3).toFixed(1)} GB ({memory.gpuPercent}%) · CPU/RAM:{' '}
                                  {(memory.cpuBytes / 1024 ** 3).toFixed(1)} GB ({memory.cpuPercent}%)
                                </div>
                              )}
                        ...
                        {isUpdatingThis && (
                          <div className="p-2.5 rounded-lg bg-slate-950/80 border border-amber-500/40 space-y-1.5 font-mono text-[11px]">
                            ...
                            <div className="flex justify-between text-[10px] text-slate-400">
                              ...
                              <span>
                                {downloadProgress.mbCompleted} / {downloadProgress.mbTotal} MB
                              </span>
                            </div>
                          </div>
                        )}
                        ...
                        <div className="grid grid-cols-2 gap-2 text-[11px] font-mono bg-slate-950/70 p-2.5 rounded-xl border border-slate-800/80">
                          ...
                          <div>
                            ...
                            <span className="font-bold text-slate-200">{m?.sizeBytes ? `${(m.sizeBytes / 1024 ** 3).toFixed(2)} GB` : 'N/D'}</span>
                          </div>
                          <div>
        ...
        <section className="space-y-4">
          ...
          <AgentExecutionLimitsConfig settings={settings} onUpdateSettings={onUpdateSettings} isActive={isActive} />
          ...
          <div className="glass-panel rounded-xl p-5 border border-slate-800 space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-800 pt-4">
              ...
              >
                <Trash2 className="h-3.5 w-3.5" /> {t('settings.clearCodingAgentDebugLog')}
                {auditLogCleared ? ' ✓' : ''}
              </button>
            </div>
          </div>
        ...
        <div className="glass-panel rounded-xl p-5 border border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400 max-w-xl">{t('settings.aboutSectionDescription')}</p>
          </div>
