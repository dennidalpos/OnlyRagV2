    ...
    <div className="space-y-4">
      {isInitialSetup && (
        <div className="p-3.5 rounded-xl bg-cyan-950/40 border border-cyan-500/40 space-y-1 shadow-md">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold text-cyan-200 uppercase tracking-wider">{t('hardwareWizard.initialWelcomeBadge')}</h3>
          </div>
          <p className="text-xs text-slate-300">{t('hardwareWizard.modalSubtitle')}</p>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
          ...
          <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
            ...
            <span className="font-semibold text-emerald-300 font-mono">
              {recommendations.safeVramBudgetGB > 0 ? `${recommendations.safeVramBudgetGB.toFixed(1)} GB Net (Buffer 25% + 1.5GB OS)` : 'CPU / RAM Bound'}
            </span>
          </div>
          <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
      ...
      <div
        className={`p-3.5 rounded-xl border flex items-center justify-between transition-all ${
          diagnostics?.ollama.status === 'online' ? 'bg-emerald-950/30 border-emerald-500/50' : 'bg-rose-950/30 border-rose-500/50'
        }`}
      >
        <div className="flex items-center gap-3">
          <Zap className={`w-5 h-5 ${diagnostics?.ollama.status === 'online' ? 'text-emerald-400' : 'text-rose-400'}`} />
          <div>
        ...
        {diagnostics?.ollama.status !== 'online' && !isRemoteMode && (
          ...
          >
            <Download className="w-3.5 h-3.5" /> {isInstallingOllama ? t('common.loading') : t('sidebar.installLaunchOllama')}
          </button>
        )}
        {diagnostics?.ollama.status !== 'online' && isRemoteMode && (
